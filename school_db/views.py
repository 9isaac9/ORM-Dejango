...
def problem_one(request):
    straight_a_student = Student.objects.filter(gpa__gt=3.0).order_by('-gpa')
    for a_grade in straight_a_student:
      print(f'First Name: {a_grade.first_name} Last Name: {a_grade.last_name} GPA: {a_grade.gpa}')
...
def problem_two(request):
    hire_dates = Instructor.objects.filter(hire_date__year__lt=2010).order_by('hire_date')
    for hiring_dates in hire_dates:
      print(f'First Name: {hiring_dates.first_name} Last Name: {hiring_dates.last_name} {hiring_dates.hire_date}')
    return complete(request)
...
def problem_four(request):
    ...
    print('Courses Count:', course_count)

    instructor_count = Instructor.objects.all().count()
    print('Instructors Count:', instructor_count)
